...
namespace UDIL.Read
{
    public partial class MDI : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    mdiPrivateKey.Text = SessionManager.PrivateKey;
                }

                // Populate global device ID from session if available
                if (SessionManager.HasGlobalDeviceId)
                {
                    mdiGlobalDeviceId.Text = SessionManager.GlobalDeviceId;
                }

                // Generate unique transaction ID
                mdiTransactionId.Text = AppCommon.GenerateTransactionId();
            }
        }

        protected void btnMdiRead_Click(object sender, EventArgs e)
        {
            lblMdiMessage.Text = string.Empty;
            pnlResponse.Visible = false;

            string transactionId = mdiTransactionId.Text.Trim();
            string globalDeviceId = mdiGlobalDeviceId.Text.Trim();
            string type = mdiType.SelectedValue;

            string privateKey = SessionManager.PrivateKey;

            if (string.IsNullOrEmpty(privateKey))
            {
                lblMdiMessage.Text = "Please authorize first to obtain a private key.";
                lblMdiMessage.CssClass = "text-danger";
                return;
            }

            mdiPrivateKey.Text = privateKey;

            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId) || string.IsNullOrEmpty(type))
            {
                lblMdiMessage.Text = "All fields are required: Transaction ID, Global Device ID, and Type.";
                lblMdiMessage.CssClass = "text-danger";
                return;
            }
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting MDI read for transaction: {transactionId}");

                MdiReadResponse response = GetMdiData(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] GetMdiData returned: {(response == null ? "NULL" : $"status={response.status}, message={response.message}")}");

                if (response == null)
                {
                    lblMdiMessage.Text = "Failed to get a response from the MDI read endpoint.";
                    lblMdiMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblMdiMessage.Text = $"MDI read failed: {response.message}";
                    lblMdiMessage.CssClass = "text-danger";
                ...
                // Success
                lblMdiMessage.Text = $"MDI read succeeded. {response.message}";
                lblMdiMessage.CssClass = "text-success";
                ...
                if (response.data != null && response.data.Count > 0)
                {
                    ...
                    lblRespMdiDateTime.Text = !string.IsNullOrEmpty(data.mdi_datetime) ? data.mdi_datetime : "N/A";
                    lblRespMdiResetDate.Text = !string.IsNullOrEmpty(data.mdi_reset_date) ? data.mdi_reset_date : "N/A";
                    lblRespMdiResetTime.Text = !string.IsNullOrEmpty(data.mdi_reset_time) ? data.mdi_reset_time : "N/A";
                }
                else
                {
                    lblRespGlobalDeviceId.Text = "N/A";
                    lblRespMsn.Text = "N/A";
                    lblRespMdiDateTime.Text = "N/A";
                    lblRespMdiResetDate.Text = "N/A";
                    lblRespMdiResetTime.Text = "N/A";
                }
                ...
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableMdiBtn", "disableMdiButton();", true);
            }
            catch (WebException ex)
            {
                lblMdiMessage.Text = "HTTP Error: " + ex.Message;
                lblMdiMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblMdiMessage.Text = "Error: " + ex.Message;
                lblMdiMessage.CssClass = "text-danger";
            }
        }

        private MdiReadResponse GetMdiData(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<MdiReadResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"MDI read request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class MdiReadResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public List<MdiData> data { get; set; }
            public string message { get; set; }
        }

        public class MdiData
        {
            public string global_device_id { get; set; }
            public string msn { get; set; }
            public string mdi_datetime { get; set; }
            public string mdi_reset_date { get; set; }
            public string mdi_reset_time { get; set; }
        }
        ...
        protected void btnMdiResponsePass_Click(object sender, EventArgs e)
        {
            try
            {
                string transactionId = SessionManager.CurrentTransactionId;
                SaveApiTestResult(transactionId, "Pass", "MDI Read Response marked as Pass");

                btnMdiResponsePass.CssClass = "btn btn-success btn-sm disabled";
                btnMdiResponsePass.Enabled = false;
                btnMdiResponseFail.Visible = false;

                lblMdiMessage.Text = "MDI Response marked as Pass.";
                lblMdiMessage.CssClass = "text-success";
            }
            catch (Exception ex)
            {
        ...
        protected void btnMdiResponseFail_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)mdiResponseRemarks).Style.Add("display", "block");
                txtMdiResponseRemarks.Text = "";
                txtMdiResponseRemarks.Focus();
            }
            catch (Exception ex)
            {
        ...
        protected void btnSaveMdiResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                string remarks = txtMdiResponseRemarks.Text.Trim();
                if (!string.IsNullOrEmpty(remarks))
                {
                    string transactionId = SessionManager.CurrentTransactionId;
                    string globalDeviceId = SessionManager.GlobalDeviceId;

                    SaveTestResult("MDIReadResponse", "Fail", remarks, transactionId, globalDeviceId);

                    ((System.Web.UI.HtmlControls.HtmlControl)mdiResponseRemarks).Style.Add("display", "none");

                    btnMdiResponsePass.Visible = false;
                    btnMdiResponseFail.CssClass = "btn btn-danger btn-sm disabled";
                    btnMdiResponseFail.Enabled = false;

                    lblMdiMessage.Text = $"MDI Response marked as Fail. Reason: {remarks}";
                    lblMdiMessage.CssClass = "text-danger";

                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "showResponseFailAlert",
                        $"alert('MDI Response marked as Fail with remarks: " + remarks.Replace("'", "\\'") + "');", true);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnCancelMdiResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)mdiResponseRemarks).Style.Add("display", "none");
