...
namespace UDIL.Read
{
    public partial class OPPO : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    oppoPrivateKey.Text = SessionManager.PrivateKey;
                }

                // Populate global device ID from session if available
                if (SessionManager.HasGlobalDeviceId)
                {
                    oppoGlobalDeviceId.Text = SessionManager.GlobalDeviceId;
                }

                // Generate unique transaction ID
                oppoTransactionId.Text = AppCommon.GenerateTransactionId();
            }
        }

        protected void btnOppoRead_Click(object sender, EventArgs e)
        {
            lblOppoMessage.Text = string.Empty;
            pnlResponse.Visible = false;

            string transactionId = oppoTransactionId.Text.Trim();
            string globalDeviceId = oppoGlobalDeviceId.Text.Trim();
            string type = oppoType.SelectedValue;

            string privateKey = SessionManager.PrivateKey;

            if (string.IsNullOrEmpty(privateKey))
            {
                lblOppoMessage.Text = "Please authorize first to obtain a private key.";
                lblOppoMessage.CssClass = "text-danger";
                return;
            }

            oppoPrivateKey.Text = privateKey;

            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId) || string.IsNullOrEmpty(type))
            {
                lblOppoMessage.Text = "All fields are required: Transaction ID, Global Device ID, and Type.";
                lblOppoMessage.CssClass = "text-danger";
                return;
            }
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting OPPO read for transaction: {transactionId}");

                OppoReadResponse response = GetOppoData(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] GetOppoData returned: {(response == null ? "NULL" : $"status={response.status}, message={response.message}")}");

                if (response == null)
                {
                    lblOppoMessage.Text = "Failed to get a response from the OPPO read endpoint.";
                    lblOppoMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblOppoMessage.Text = $"OPPO read failed: {response.message}";
                    lblOppoMessage.CssClass = "text-danger";
                ...
                // Success
                lblOppoMessage.Text = $"OPPO read succeeded. {response.message}";
                lblOppoMessage.CssClass = "text-success";
                ...
                if (response.data != null && response.data.Count > 0)
                {
                    ...
                    lblRespOppoDateTime.Text = !string.IsNullOrEmpty(data.oppo_datetime) ? data.oppo_datetime : "N/A";
                    lblRespOppoOpticalPortOnDateTime.Text = !string.IsNullOrEmpty(data.oppo_optical_port_on_datetime) ? data.oppo_optical_port_on_datetime : "N/A";
                    lblRespOppoOpticalPortOffDateTime.Text = !string.IsNullOrEmpty(data.oppo_optical_port_off_datetime) ? data.oppo_optical_port_off_datetime : "N/A";
                }
                else
                {
                    lblRespGlobalDeviceId.Text = "N/A";
                    lblRespMsn.Text = "N/A";
                    lblRespOppoDateTime.Text = "N/A";
                    lblRespOppoOpticalPortOnDateTime.Text = "N/A";
                    lblRespOppoOpticalPortOffDateTime.Text = "N/A";
                }
                ...
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableOppoBtn", "disableOppoButton();", true);
            }
            catch (WebException ex)
            {
                lblOppoMessage.Text = "HTTP Error: " + ex.Message;
                lblOppoMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblOppoMessage.Text = "Error: " + ex.Message;
                lblOppoMessage.CssClass = "text-danger";
            }
        }

        private OppoReadResponse GetOppoData(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<OppoReadResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"OPPO read request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class OppoReadResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public List<OppoData> data { get; set; }
        ...
        public class OppoData
        {
            ...
            public string oppo_datetime { get; set; }
            public string oppo_optical_port_on_datetime { get; set; }
            public string oppo_optical_port_off_datetime { get; set; }
        }
        ...
        protected void btnOppoResponsePass_Click(object sender, EventArgs e)
        {
            try
            {
                string transactionId = SessionManager.CurrentTransactionId;
                SaveApiTestResult(transactionId, "Pass", "OPPO Read Response marked as Pass");

                btnOppoResponsePass.CssClass = "btn btn-success btn-sm disabled";
                btnOppoResponsePass.Enabled = false;
                btnOppoResponseFail.Visible = false;

                lblOppoMessage.Text = "OPPO Response marked as Pass.";
                lblOppoMessage.CssClass = "text-success";
            }
            catch (Exception ex)
            {
        ...
        protected void btnOppoResponseFail_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)oppoResponseRemarks).Style.Add("display", "block");
                txtOppoResponseRemarks.Text = "";
                txtOppoResponseRemarks.Focus();
            }
            catch (Exception ex)
            {
        ...
        protected void btnSaveOppoResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                string remarks = txtOppoResponseRemarks.Text.Trim();
                if (!string.IsNullOrEmpty(remarks))
                {
                    string transactionId = SessionManager.CurrentTransactionId;
                    string globalDeviceId = SessionManager.GlobalDeviceId;

                    SaveTestResult("OPPOReadResponse", "Fail", remarks, transactionId, globalDeviceId);

                    ((System.Web.UI.HtmlControls.HtmlControl)oppoResponseRemarks).Style.Add("display", "none");

                    btnOppoResponsePass.Visible = false;
                    btnOppoResponseFail.CssClass = "btn btn-danger btn-sm disabled";
                    btnOppoResponseFail.Enabled = false;

                    lblOppoMessage.Text = $"OPPO Response marked as Fail. Reason: {remarks}";
                    lblOppoMessage.CssClass = "text-danger";

                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "showResponseFailAlert",
                        $"alert('OPPO Response marked as Fail with remarks: " + remarks.Replace("'", "\\'") + "');", true);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnCancelOppoResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)oppoResponseRemarks).Style.Add("display", "none");
