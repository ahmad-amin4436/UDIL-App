...
namespace UDIL.Read
{
    public partial class AUXR : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    auxrPrivateKey.Text = Session["PrivateKey"].ToString();
                }
        ...
        protected void btnAuxrRead_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting AUXR read for transaction: {transactionId}");

                AuxrReadResponse response = GetAuxrData(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] GetAuxrData returned: {(response == null ? "NULL" : $"status={response.status}, message={response.message}")}");

                if (response == null)
                {
                    lblAuxrMessage.Text = "Failed to get a response from the AUXR read endpoint.";
                    lblAuxrMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblAuxrMessage.Text = $"AUXR read failed: {response.message}";
                    lblAuxrMessage.CssClass = "text-danger";
                ...
                // Success
                lblAuxrMessage.Text = $"AUXR read succeeded. {response.message}";
        ...
        private AuxrReadResponse GetAuxrData(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<AuxrReadResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"AUXR read request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class AuxrReadResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public List<AuxrData> data { get; set; }
            public string message { get; set; }
        }

        public class AuxrData
        {
            public string global_device_id { get; set; }
            public string msn { get; set; }
            public string auxr_datetime { get; set; }
            public string auxr_status { get; set; }
        }
        ...
        protected void btnAuxrResponsePass_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SaveApiTestResult(transactionId, "Pass", "AUXR Read Response marked as Pass");
                ...
                lblAuxrMessage.Text = "AUXR Response marked as Pass.";
                lblAuxrMessage.CssClass = "text-success";
            }
            catch (Exception ex)
            {
        ...
        protected void btnSaveAuxrResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(remarks))
                {
                    ...
                    SaveTestResult("AUXRReadResponse", "Fail", remarks, transactionId, globalDeviceId);
                    ...
                    lblAuxrMessage.Text = $"AUXR Response marked as Fail. Reason: {remarks}";
                    lblAuxrMessage.CssClass = "text-danger";

                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "showResponseFailAlert",
                        $"alert('AUXR Response marked as Fail with remarks: {remarks.Replace("'", "\\'")}');", true);
                }
            }
            catch (Exception ex)
            {
