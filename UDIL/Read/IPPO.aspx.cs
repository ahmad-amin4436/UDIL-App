...
namespace UDIL.Read
{
    public partial class IPPO : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    ippoPrivateKey.Text = SessionManager.PrivateKey;
                }

                // Populate global device ID from session if available
                if (SessionManager.HasGlobalDeviceId)
                {
                    ippoGlobalDeviceId.Text = SessionManager.GlobalDeviceId;
                }

                // Generate unique transaction ID
                ippoTransactionId.Text = AppCommon.GenerateTransactionId();
            }
        }

        protected void btnIppoRead_Click(object sender, EventArgs e)
        {
            lblIppoMessage.Text = string.Empty;
            pnlResponse.Visible = false;

            string transactionId = ippoTransactionId.Text.Trim();
            string globalDeviceId = ippoGlobalDeviceId.Text.Trim();
            string type = ippoType.SelectedValue;

            string privateKey = SessionManager.PrivateKey;

            if (string.IsNullOrEmpty(privateKey))
            {
                lblIppoMessage.Text = "Please authorize first to obtain a private key.";
                lblIppoMessage.CssClass = "text-danger";
                return;
            }

            ippoPrivateKey.Text = privateKey;

            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId) || string.IsNullOrEmpty(type))
            {
                lblIppoMessage.Text = "All fields are required: Transaction ID, Global Device ID, and Type.";
                lblIppoMessage.CssClass = "text-danger";
                return;
            }
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting IPPO read for transaction: {transactionId}");

                IppoReadResponse response = GetIppoData(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] GetIppoData returned: {(response == null ? "NULL" : $"status={response.status}, message={response.message}")}");

                if (response == null)
                {
                    lblIppoMessage.Text = "Failed to get a response from the IPPO read endpoint.";
                    lblIppoMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblIppoMessage.Text = $"IPPO read failed: {response.message}";
                    lblIppoMessage.CssClass = "text-danger";
                ...
                // Success
                lblIppoMessage.Text = $"IPPO read succeeded. {response.message}";
                lblIppoMessage.CssClass = "text-success";
                ...
                if (response.data != null && response.data.Count > 0)
                {
                    ...
                    lblRespIppoDateTime.Text = !string.IsNullOrEmpty(data.ippo_datetime) ? data.ippo_datetime : "N/A";
                    lblRespPrimaryIpAddress.Text = !string.IsNullOrEmpty(data.ippo_primary_ip_address) ? data.ippo_primary_ip_address : "N/A";
                    lblRespSecondaryIpAddress.Text = !string.IsNullOrEmpty(data.ippo_secondary_ip_address) ? data.ippo_secondary_ip_address : "N/A";
                    lblRespPrimaryPort.Text = !string.IsNullOrEmpty(data.ippo_primary_port) ? data.ippo_primary_port : "N/A";
                    lblRespSecondaryPort.Text = !string.IsNullOrEmpty(data.ippo_secondary_port) ? data.ippo_secondary_port : "N/A";
                }
                else
                {
                    lblRespGlobalDeviceId.Text = "N/A";
                    lblRespMsn.Text = "N/A";
                    lblRespIppoDateTime.Text = "N/A";
                    lblRespPrimaryIpAddress.Text = "N/A";
                    lblRespSecondaryIpAddress.Text = "N/A";
                    lblRespPrimaryPort.Text = "N/A";
                    lblRespSecondaryPort.Text = "N/A";
                }
                ...
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableIppoBtn", "disableIppoButton();", true);
            }
            catch (WebException ex)
            {
                lblIppoMessage.Text = "HTTP Error: " + ex.Message;
                lblIppoMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblIppoMessage.Text = "Error: " + ex.Message;
                lblIppoMessage.CssClass = "text-danger";
            }
        }

        private IppoReadResponse GetIppoData(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<IppoReadResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"IPPO read request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class IppoReadResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public List<IppoData> data { get; set; }
            public string message { get; set; }
        }

        public class IppoData
        {
            public string global_device_id { get; set; }
            public string msn { get; set; }
            public string ippo_datetime { get; set; }
            public string ippo_primary_ip_address { get; set; }
            public string ippo_secondary_ip_address { get; set; }
            public string ippo_primary_port { get; set; }
            public string ippo_secondary_port { get; set; }
        }
        ...
        protected void btnIppoResponsePass_Click(object sender, EventArgs e)
        {
            try
            {
                string transactionId = SessionManager.CurrentTransactionId;
                SaveApiTestResult(transactionId, "Pass", "IPPO Read Response marked as Pass");

                btnIppoResponsePass.CssClass = "btn btn-success btn-sm disabled";
                btnIppoResponsePass.Enabled = false;
                btnIppoResponseFail.Visible = false;

                lblIppoMessage.Text = "IPPO Response marked as Pass.";
                lblIppoMessage.CssClass = "text-success";
            }
            catch (Exception ex)
            {
        ...
        protected void btnIppoResponseFail_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)ippoResponseRemarks).Style.Add("display", "block");
                txtIppoResponseRemarks.Text = "";
                txtIppoResponseRemarks.Focus();
            }
            catch (Exception ex)
            {
        ...
        protected void btnSaveIppoResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                string remarks = txtIppoResponseRemarks.Text.Trim();
                if (!string.IsNullOrEmpty(remarks))
                {
                    string transactionId = SessionManager.CurrentTransactionId;
                    string globalDeviceId = SessionManager.GlobalDeviceId;

                    SaveTestResult("IPPOReadResponse", "Fail", remarks, transactionId, globalDeviceId);

                    ((System.Web.UI.HtmlControls.HtmlControl)ippoResponseRemarks).Style.Add("display", "none");

                    btnIppoResponsePass.Visible = false;
                    btnIppoResponseFail.CssClass = "btn btn-danger btn-sm disabled";
                    btnIppoResponseFail.Enabled = false;

                    lblIppoMessage.Text = $"IPPO Response marked as Fail. Reason: {remarks}";
                    lblIppoMessage.CssClass = "text-danger";

                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "showResponseFailAlert",
                        $"alert('IPPO Response marked as Fail with remarks: {remarks.Replace("'", "\\\'")}');", true);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnCancelIppoResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)ippoResponseRemarks).Style.Add("display", "none");
