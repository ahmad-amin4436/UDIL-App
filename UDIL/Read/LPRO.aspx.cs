...
namespace UDIL.Read
{
    public partial class LPRO : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    lproPrivateKey.Text = SessionManager.PrivateKey;
                }
        ...
        protected void btnLproRead_Click(object sender, EventArgs e)
        {
            lblLproMessage.Text = string.Empty;
            ...
            {
                lblLproMessage.Text = "Please authorize first to obtain a private key.";
                lblLproMessage.CssClass = "text-danger";
                return;
            }

            lproPrivateKey.Text = privateKey;
            ...
            {
                lblLproMessage.Text = "All fields are required: Transaction ID, Global Device ID, Type, Start DateTime, and End DateTime.";
                lblLproMessage.CssClass = "text-danger";
                return;
            }
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting LPRO read for transaction: {transactionId}");

                LproReadResponse response = GetLproData(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] GetLproData returned: {(response == null ? "NULL" : $"status={response.status}, message={response.message}")}");

                if (response == null)
                {
                    lblLproMessage.Text = "Failed to get a response from the LPRO read endpoint.";
                    lblLproMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblLproMessage.Text = $"LPRO read failed: {response.message}";
                    lblLproMessage.CssClass = "text-danger";
                ...
                // Success
                lblLproMessage.Text = $"LPRO read succeeded. {response.message}";
                lblLproMessage.CssClass = "text-success";
                ...
                {
                    gvLproData.DataSource = null;
                    gvLproData.DataBind();
                }

                pnlResponse.Visible = true;
                pnlLproData.Visible = true;
                ...
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableLproBtn", "disableLproButton();", true);
            }
            catch (WebException ex)
            {
                lblLproMessage.Text = "HTTP Error: " + ex.Message;
                lblLproMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblLproMessage.Text = "Error: " + ex.Message;
                lblLproMessage.CssClass = "text-danger";
            }
        }
        ...
        private LproReadResponse GetLproData(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<LproReadResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"LPRO read request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        protected void btnLproResponsePass_Click(object sender, EventArgs e)
        {
            try
            {
                string transactionId = SessionManager.CurrentTransactionId;
                SaveApiTestResult(transactionId, "Pass", "LPRO Read Response marked as Pass");

                btnLproResponsePass.CssClass = "btn btn-success btn-sm disabled";
                btnLproResponsePass.Enabled = false;
                btnLproResponseFail.Visible = false;

                lblLproMessage.Text = "LPRO Response marked as Pass.";
                lblLproMessage.CssClass = "text-success";
            }
            catch (Exception ex)
            {
        ...
        protected void btnLproResponseFail_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)lproResponseRemarks).Style.Add("display", "block");
                txtLproResponseRemarks.Text = "";
                txtLproResponseRemarks.Focus();
            }
            catch (Exception ex)
            {
        ...
        protected void btnSaveLproResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                string remarks = txtLproResponseRemarks.Text.Trim();
                if (!string.IsNullOrEmpty(remarks))
                {
                    string transactionId = SessionManager.CurrentTransactionId;
                    string globalDeviceId = SessionManager.GlobalDeviceId;

                    SaveTestResult("LPROReadResponse", "Fail", remarks, transactionId, globalDeviceId);

                    ((System.Web.UI.HtmlControls.HtmlControl)lproResponseRemarks).Style.Add("display", "none");

                    btnLproResponsePass.Visible = false;
                    btnLproResponseFail.CssClass = "btn btn-danger btn-sm disabled";
                    btnLproResponseFail.Enabled = false;

                    lblLproMessage.Text = $"LPRO Response marked as Fail. Reason: {remarks}";
                    lblLproMessage.CssClass = "text-danger";

                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "showResponseFailAlert",
                        $"alert('LPRO Response marked as Fail with remarks: {remarks.Replace("'", "\\'")}');", true);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnCancelLproResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)lproResponseRemarks).Style.Add("display", "none");
