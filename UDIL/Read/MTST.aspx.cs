...
namespace UDIL.Read
{
    public partial class MTST : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    mtstPrivateKey.Text = SessionManager.PrivateKey;
                }

                // Populate global device ID from session if available
                if (SessionManager.HasGlobalDeviceId)
                {
                    mtstGlobalDeviceId.Text = SessionManager.GlobalDeviceId;
                }

                // Generate unique transaction ID
                mtstTransactionId.Text = AppCommon.GenerateTransactionId();
            }
        }

        protected void btnMtstRead_Click(object sender, EventArgs e)
        {
            lblMtstMessage.Text = string.Empty;
            pnlResponse.Visible = false;

            string transactionId = mtstTransactionId.Text.Trim();
            string globalDeviceId = mtstGlobalDeviceId.Text.Trim();
            string type = mtstType.SelectedValue;

            string privateKey = SessionManager.PrivateKey;

            if (string.IsNullOrEmpty(privateKey))
            {
                lblMtstMessage.Text = "Please authorize first to obtain a private key.";
                lblMtstMessage.CssClass = "text-danger";
                return;
            }

            mtstPrivateKey.Text = privateKey;

            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId) || string.IsNullOrEmpty(type))
            {
                lblMtstMessage.Text = "All fields are required: Transaction ID, Global Device ID, and Type.";
                lblMtstMessage.CssClass = "text-danger";
                return;
            }
            ...
            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting MTST read for transaction: {transactionId}");

                MtstReadResponse response = GetMtstData(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] GetMtstData returned: {(response == null ? "NULL" : $"status={response.status}, message={response.message}")}");

                if (response == null)
                {
                    lblMtstMessage.Text = "Failed to get a response from the MTST read endpoint.";
                    lblMtstMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblMtstMessage.Text = $"MTST read failed: {response.message}";
                    lblMtstMessage.CssClass = "text-danger";
                ...
                // Success
                lblMtstMessage.Text = $"MTST read succeeded. {response.message}";
                lblMtstMessage.CssClass = "text-success";
                ...
                if (response.data != null && response.data.Count > 0)
                {
                    ...
                    lblRespMtstDateTime.Text = !string.IsNullOrEmpty(data.mtst_datetime) ? data.mtst_datetime : "N/A";
                    lblRespMtstMeterActivationStatus.Text = !string.IsNullOrEmpty(data.mtst_meter_activation_status) ? data.mtst_meter_activation_status : "N/A";
                }
                else
                {
                    lblRespGlobalDeviceId.Text = "N/A";
                    lblRespMsn.Text = "N/A";
                    lblRespMtstDateTime.Text = "N/A";
                    lblRespMtstMeterActivationStatus.Text = "N/A";
                }
                ...
                ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableMtstBtn", "disableMtstButton();", true);
            }
            catch (WebException ex)
            {
                lblMtstMessage.Text = "HTTP Error: " + ex.Message;
                lblMtstMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblMtstMessage.Text = "Error: " + ex.Message;
                lblMtstMessage.CssClass = "text-danger";
            }
        }
        ...
        private MtstReadResponse GetMtstData(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<MtstReadResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"MTST read request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class MtstReadResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public List<MtstData> data { get; set; }
            public string message { get; set; }
        }

        public class MtstData
        {
            public string global_device_id { get; set; }
            public string msn { get; set; }
            public string mtst_datetime { get; set; }
            public string mtst_meter_activation_status { get; set; }
        }
            ...
            try
            {
                switch (tableName)
                {
                    ...
                    case "Events":
                        ...
                        break;
                    case "MTSTResponse":
                        remarks = txtMtstResponseRemarks.Text.Trim();
                        break;
                }

                if (!string.IsNullOrEmpty(remarks))
                {
                    if (tableName == "MTSTResponse")
                    {
                        SaveTestResult("MTSTReadResponse", "Fail", remarks, SessionManager.CurrentTransactionId, SessionManager.GlobalDeviceId);
                        HideRemarksSection(tableName);
                        btnMtstResponsePass.Visible = false;
                        btnMtstResponseFail.CssClass = "btn btn-danger btn-sm disabled";
                        btnMtstResponseFail.Enabled = false;
                        lblMtstMessage.Text = $"MTST Response marked as Fail. Reason: {remarks}";
                        lblMtstMessage.CssClass = "text-danger";
                    }
                    else
                    {
            ...
            try
            {
                switch (tableName)
                {
                    ...
                    case "Events":
                        ...
                        break;
                    case "MTSTResponse":
                        ((System.Web.UI.HtmlControls.HtmlControl)mtstResponseRemarks).Style.Add("display", "none");
                        txtMtstResponseRemarks.Text = "";
                        break;
                }
            }
            catch (Exception ex)
        ...
        protected void btnMtstResponsePass_Click(object sender, EventArgs e)
        {
            try
            {
                string transactionId = SessionManager.CurrentTransactionId;
                SaveApiTestResult(transactionId, "Pass", "MTST Read Response marked as Pass");

                btnMtstResponsePass.CssClass = "btn btn-success btn-sm disabled";
                btnMtstResponsePass.Enabled = false;
                btnMtstResponseFail.Visible = false;

                lblMtstMessage.Text = "MTST Response marked as Pass.";
                lblMtstMessage.CssClass = "text-success";
            }
            catch (Exception ex)
            {
        ...
        protected void btnMtstResponseFail_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)mtstResponseRemarks).Style.Add("display", "block");
                txtMtstResponseRemarks.Text = "";
                txtMtstResponseRemarks.Focus();
            }
            catch (Exception ex)
            {
        ...
        protected void btnSaveMtstResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                string remarks = txtMtstResponseRemarks.Text.Trim();
                if (!string.IsNullOrEmpty(remarks))
                {
                    string transactionId = SessionManager.CurrentTransactionId;
                    string globalDeviceId = SessionManager.GlobalDeviceId;

                    SaveTestResult("MTSTReadResponse", "Fail", remarks, transactionId, globalDeviceId);
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "showResponseFailAlert",
                        $"alert('MTST Response marked as Fail with remarks: " + remarks.Replace("'", "\\'") + "');", true);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnCancelMtstResponseRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                ((System.Web.UI.HtmlControls.HtmlControl)mtstResponseRemarks).Style.Add("display", "none");
