...
{
    public partial class TimeSync : UDIL.AuthenticatedPage
    {
        ...
        protected void btnTimeSync_Click(object sender, EventArgs e)
        {
            lblTimeSyncMessage.Text = string.Empty;
            // Store time sync request time
            Session["TimeSyncRequestTime"] = DateTime.Now;
            ...
            {
                lblTimeSyncMessage.Text = "Please authorize first to obtain a private key.";
            ...
            {
                lblTimeSyncMessage.Text = "Transaction ID, Global Device ID, and Request DateTime are required.";
                lblTimeSyncMessage.CssClass = "text-danger";
                return;
            }
            ...
            string postData = $"global_device_id={HttpUtility.UrlEncode(globalDeviceIdArray)}&request_datetime={HttpUtility.UrlEncode(requestDateTime)}";

            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting time sync for transaction: {transactionId}");

                TimeSyncResponse syncResponse = PostTimeSync(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] PostTimeSync returned: {(syncResponse == null ? "NULL" : $"status={syncResponse.status}, message={syncResponse.message}")}");

                if (syncResponse == null)
                {
                    lblTimeSyncMessage.Text = "Failed to get a response from the time sync endpoint.";
                    lblTimeSyncMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblTimeSyncMessage.Text = $"Time sync failed: {syncResponse.message}";
                    lblTimeSyncMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblTimeSyncMessage.Text = "Time sync accepted, but transaction status could not be retrieved.";
                    lblTimeSyncMessage.CssClass = "text-warning";
                    return;
                }
                ...
                if (commandCancelled)
                {
                    ...
                    lblTimeSyncMessage.Text = $"Time sync failed. Command was cancelled by the meter (indv_status: 2, status_level: 4). Status: {stageText}.";
                    lblTimeSyncMessage.CssClass = "text-danger";
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableTimeSyncBtn", "disableTimeSyncButton();", true);
                }
                else
                {
                    string stageText = AppCommon.GetStageDescription(maxStatusLevel);
                    lblTimeSyncMessage.Text = $"Time sync succeeded. Transaction status: {statusResponse.status}. Latest status level: {maxStatusLevel} ({stageText}).";
            ...
            {
                lblTimeSyncMessage.Text = "HTTP Error: " + ex.Message;
                lblTimeSyncMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblTimeSyncMessage.Text = "Error: " + ex.Message;
                lblTimeSyncMessage.CssClass = "text-danger";
            }
        }
        ...
        protected void timerTracker_Tick(object sender, EventArgs e)
        {
            ...
            foreach (var item in statusResponse.data)
            {
                ...
                if (item.indv_status == "2" && item.status_level == "4")
                {
                    ...
                    lblStageDescription.Text = "Command was cancelled by the meter (indv_status: 2, status_level: 4).";
                    lblStageDescription.CssClass = "text-danger";
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "resetTimeSyncLoading", "resetTimeSyncLoading();", true);
        ...
        private TimeSyncResponse PostTimeSync(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<TimeSyncResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"Time sync request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        protected void SaveRemarks_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    HandleTableAction(tableName + "Table - Time Sync", "Fail", remarks);
