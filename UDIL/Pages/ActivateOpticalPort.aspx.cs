...
{
    public partial class ActivateOpticalPort : UDIL.AuthenticatedPage
    {
        ...
        protected void btnActivateOpticalPort_Click(object sender, EventArgs e)
        {
            ...
            // Store Activate Optical Port request time
            Session["ActivateOpticalPortRequestTime"] = DateTime.Now;
            ...
            {
                lblActivateOpticalPortMessage.Text = "Please authorize first to obtain a private key.";
                lblActivateOpticalPortMessage.CssClass = "text-danger";
                return;
            }
            ...
            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId) || string.IsNullOrEmpty(requestDateTime) ||
                string.IsNullOrEmpty(opticalPortOnDateTime) || string.IsNullOrEmpty(opticalPortOffDateTime))
            {
                lblActivateOpticalPortMessage.Text = "All fields are required: Transaction ID, Global Device ID, Request DateTime, Optical Port On DateTime, and Optical Port Off DateTime.";
                lblActivateOpticalPortMessage.CssClass = "text-danger";
                return;
            }
            ...
            string postData = $"global_device_id={HttpUtility.UrlEncode(globalDeviceIdArray)}&request_datetime={HttpUtility.UrlEncode(requestDateTime)}&optical_port_on_datetime={HttpUtility.UrlEncode(opticalPortOnDateTime)}&optical_port_off_datetime={HttpUtility.UrlEncode(opticalPortOffDateTime)}";

            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting Activate Optical Port for transaction: {transactionId}");

                ActivateOpticalPortResponse activateOpticalResponse = PostActivateOpticalPort(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] PostActivateOpticalPort returned: {(activateOpticalResponse == null ? "NULL" : $"status={activateOpticalResponse.status}, message={activateOpticalResponse.message}")}");

                if (activateOpticalResponse == null)
                {
                    lblActivateOpticalPortMessage.Text = "Failed to get a response from the activate optical port endpoint.";
                    lblActivateOpticalPortMessage.CssClass = "text-danger";
                    return;
                }

                if (activateOpticalResponse.status != "1")
                {
                    lblActivateOpticalPortMessage.Text = $"Activate optical port failed: {activateOpticalResponse.message}";
                    lblActivateOpticalPortMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblActivateOpticalPortMessage.Text = "Activate optical port accepted, but transaction status could not be retrieved.";
                    lblActivateOpticalPortMessage.CssClass = "text-warning";
                    return;
                }
                ...
                if (commandCancelled)
                {
                    ...
                    lblActivateOpticalPortMessage.Text = $"Activate optical port failed. Command was cancelled by the meter (indv_status: 2, status_level: 4). Status: {stageText}.";
                    lblActivateOpticalPortMessage.CssClass = "text-danger";
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableActivateOpticalPortBtn", "disableActivateOpticalPortButton();", true);
                }
                else
                {
                    string stageText = AppCommon.GetStageDescription(maxStatusLevel);
                    lblActivateOpticalPortMessage.Text = $"Activate optical port succeeded. Transaction status: {statusResponse.status}. Latest status level: {maxStatusLevel} ({stageText}).";
                    lblActivateOpticalPortMessage.CssClass = "text-success";
            ...
            {
                lblActivateOpticalPortMessage.Text = "HTTP Error: " + ex.Message;
                lblActivateOpticalPortMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblActivateOpticalPortMessage.Text = "Error: " + ex.Message;
                lblActivateOpticalPortMessage.CssClass = "text-danger";
            }
        }
        ...
        protected void timerTracker_Tick(object sender, EventArgs e)
        {
            ...
            foreach (var item in statusResponse.data)
            {
                ...
                if (item.indv_status == "2" && item.status_level == "4")
                {
                    ...
                    lblStageDescription.Text = "Command was cancelled by the meter (indv_status: 2, status_level: 4).";
                    lblStageDescription.CssClass = "text-danger";
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "resetActivateOpticalPortLoading", "resetActivateOpticalPortLoading();", true);
        ...
        private ActivateOpticalPortResponse PostActivateOpticalPort(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<ActivateOpticalPortResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"Activate optical port request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class ActivateOpticalPortResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public string message { get; set; }
        ...
        protected void SaveRemarks_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    HandleTableAction(tableName + "Table - Activate Optical Port", "Fail", remarks);
        ...
        private void SaveApiTestResult(string transactionId, string status, string remarks)
        {
            try
            {
                ...
                UDIL.DAL.TestResult testResult = new UDIL.DAL.TestResult
                {
                    ...
                    TestName = "ActivateOpticalPortAPI",
                    TestType = "API",
                    Status = status,
                    Remarks = remarks,
