...
{
    public partial class MeterDataSampling : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                tsActivationDateTime.Text = DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss");
        ...
        protected void btnMeterDataSampling_Click(object sender, EventArgs e)
        {
            lblMeterDataSamplingMessage.Text = string.Empty;
            // Store Meter Data Sampling request time
            Session["MeterDataSamplingRequestTime"] = DateTime.Now;
            ...
            {
                lblMeterDataSamplingMessage.Text = "Please authorize first to obtain a private key.";
            ...
            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId) || string.IsNullOrEmpty(requestDateTime) ||
                string.IsNullOrEmpty(activationDateTime) || string.IsNullOrEmpty(dataType) || string.IsNullOrEmpty(samplingInterval) ||
                string.IsNullOrEmpty(samplingInitialTime))
            {
                lblMeterDataSamplingMessage.Text = "All fields are required: Transaction ID, Global Device ID, Request DateTime, Activation DateTime, Data Type, Sampling Interval, and Sampling Initial Time.";
                lblMeterDataSamplingMessage.CssClass = "text-danger";
                return;
            }
            ...
            string postData = $"global_device_id={HttpUtility.UrlEncode(globalDeviceIdArray)}&request_datetime={HttpUtility.UrlEncode(requestDateTime)}&activation_datetime={HttpUtility.UrlEncode(activationDateTime)}&data_type={HttpUtility.UrlEncode(dataType)}&sampling_interval={HttpUtility.UrlEncode(samplingInterval)}&sampling_initial_time={HttpUtility.UrlEncode(samplingInitialTime)}";

            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting Meter Data Sampling for transaction: {transactionId}");

                MeterDataSamplingResponse meterDataResponse = PostMeterDataSampling(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] PostMeterDataSampling returned: {(meterDataResponse == null ? "NULL" : $"status={meterDataResponse.status}, message={meterDataResponse.message}")}");

                if (meterDataResponse == null)
                {
                    lblMeterDataSamplingMessage.Text = "Failed to get a response from the meter data sampling endpoint.";
                    lblMeterDataSamplingMessage.CssClass = "text-danger";
                    return;
                }

                if (meterDataResponse.status != "1")
                {
                    lblMeterDataSamplingMessage.Text = $"Meter data sampling failed: {meterDataResponse.message}";
                    lblMeterDataSamplingMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblMeterDataSamplingMessage.Text = "Meter data sampling accepted, but transaction status could not be retrieved.";
                    lblMeterDataSamplingMessage.CssClass = "text-warning";
                    return;
                }
                ...
                if (commandCancelled)
                {
                    ...
                    lblMeterDataSamplingMessage.Text = $"Meter data sampling failed. Command was cancelled by the meter (indv_status: 2, status_level: 4). Status: {stageText}.";
                    lblMeterDataSamplingMessage.CssClass = "text-danger";
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableMeterDataSamplingBtn", "disableMeterDataSamplingButton();", true);
                }
                else
                {
                    string stageText = AppCommon.GetStageDescription(maxStatusLevel);
                    lblMeterDataSamplingMessage.Text = $"Meter data sampling succeeded. Transaction status: {statusResponse.status}. Latest status level: {maxStatusLevel} ({stageText}).";
                    lblMeterDataSamplingMessage.CssClass = "text-success";
            ...
            {
                lblMeterDataSamplingMessage.Text = "HTTP Error: " + ex.Message;
                lblMeterDataSamplingMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
        ...
        protected void timerTracker_Tick(object sender, EventArgs e)
        {
            ...
            foreach (var item in statusResponse.data)
            {
                ...
                if (item.indv_status == "2" && item.status_level == "4")
                {
                    ...
                    lblStageDescription.Text = "Command was cancelled by the meter (indv_status: 2, status_level: 4).";
                    lblStageDescription.CssClass = "text-danger";
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "resetMeterDataSamplingLoading", "resetMeterDataSamplingLoading();", true);
        ...
        private MeterDataSamplingResponse PostMeterDataSampling(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<MeterDataSamplingResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"Meter data sampling request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class MeterDataSamplingResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public string message { get; set; }
        ...
        protected void SaveRemarks_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    HandleTableAction(tableName + "Table - Meter Data Sampling", "Fail", remarks);
