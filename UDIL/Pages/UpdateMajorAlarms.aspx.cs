...
{
    public partial class UpdateMajorAlarms : UDIL.AuthenticatedPage
    {
        ...
        protected void btnUpdateMajorAlarms_Click(object sender, EventArgs e)
        {
            lblWakeSimNumberMessage.Text = string.Empty;
            // Store Update Major Alarms request time
            Session["UpdateMajorAlarmsRequestTime"] = DateTime.Now;
            ...
            // Add alarms from individual fields if they have values
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode1.Text.Trim(), wsEventDescription1.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode2.Text.Trim(), wsEventDescription2.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode3.Text.Trim(), wsEventDescription3.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode4.Text.Trim(), wsEventDescription4.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode5.Text.Trim(), wsEventDescription5.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode6.Text.Trim(), wsEventDescription6.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode7.Text.Trim(), wsEventDescription7.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode8.Text.Trim(), wsEventDescription8.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode9.Text.Trim(), wsEventDescription9.Text.Trim());
            AddAlarmIfNotEmpty(majorAlarms, wsEventCode10.Text.Trim(), wsEventDescription10.Text.Trim());
            
            // Convert to JSON string
            string majorAlarmsList = ConvertToJson(majorAlarms);
            ...
            {
                lblWakeSimNumberMessage.Text = "All required fields must be filled: Transaction ID, Global Device ID, and Request DateTime.";
                lblWakeSimNumberMessage.CssClass = "text-danger";
                return;
            }

            // Check if at least one alarm is configured
            if (majorAlarms.Count == 0)
            {
                lblWakeSimNumberMessage.Text = "At least one major alarm must be configured (both event code and description).";
                lblWakeSimNumberMessage.CssClass = "text-danger";
                return;
            }

            string postData = $"global_device_id={HttpUtility.UrlEncode(globalDeviceIdArray)}&request_datetime={HttpUtility.UrlEncode(requestDateTime)}&major_alarms_list={HttpUtility.UrlEncode(majorAlarmsList)}";

            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting Update Major Alarms for transaction: {transactionId}");

                UpdateMajorAlarmsResponse updateMajorAlarmsResponse = PostUpdateMajorAlarms(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] PostUpdateMajorAlarms returned: {(updateMajorAlarmsResponse == null ? "NULL" : $"status={updateMajorAlarmsResponse.status}, message={updateMajorAlarmsResponse.message}")}");

                if (updateMajorAlarmsResponse == null)
                {
                    lblWakeSimNumberMessage.Text = "Failed to get a response from update major alarms endpoint.";
                    lblWakeSimNumberMessage.CssClass = "text-danger";
                    return;
                }

                if (updateMajorAlarmsResponse.status != "1")
                {
                    lblWakeSimNumberMessage.Text = $"Update Major Alarms failed: {updateMajorAlarmsResponse.message}";
                    lblWakeSimNumberMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblWakeSimNumberMessage.Text = "Update Major Alarms accepted, but transaction status could not be retrieved.";
                ...
                if (commandCancelled)
                {
                    ...
                    lblWakeSimNumberMessage.Text = $"Update Major Alarms failed. Command was cancelled by the meter (indv_status: 2, status_level: 4). Status: {stageText}.";
                    lblWakeSimNumberMessage.CssClass = "text-danger";
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableUpdateMajorAlarmsBtn", "disableUpdateMajorAlarmsButton();", true);
                }
                else
                {
                    string stageText = AppCommon.GetStageDescription(maxStatusLevel);
                    lblWakeSimNumberMessage.Text = $"Update Major Alarms succeeded. Transaction status: {statusResponse.status}. Latest status level: {maxStatusLevel} ({stageText}).";
        ...
        private void AddAlarmIfNotEmpty(List<Dictionary<string, string>> alarms, string eventCode, string eventDescription)
        {
            if (!string.IsNullOrEmpty(eventCode) && !string.IsNullOrEmpty(eventDescription))
            {
                alarms.Add(new Dictionary<string, string>
                {
        ...
        private string ConvertToJson(List<Dictionary<string, string>> alarms)
        {
            if (alarms.Count == 0)
            {
                return "[]";
            ...
            foreach (var alarm in alarms)
            {
                ...
                var eventCode = EscapeJson(alarm["event_code"]);
                var eventDescription = EscapeJson(alarm["event_description"]);
                var jsonElement = $"{{\"event_code\": \"{eventCode}\",\"event_description\": \"{eventDescription}\"}}";
        ...
        private UpdateMajorAlarmsResponse PostUpdateMajorAlarms(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<UpdateMajorAlarmsResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"Update Major Alarms request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class UpdateMajorAlarmsResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public string message { get; set; }
        ...
        protected void SaveRemarks_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    HandleTableAction(tableName + "Table - Wake SIM Number", "Fail", remarks);
