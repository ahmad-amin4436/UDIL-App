...
{
    public partial class TXN : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txnPrivateKey.Text = SessionManager.PrivateKey;
                }
        ...
        protected void btnTXNCancellation_Click(object sender, EventArgs e)
        {
            lblTXNCancellationMessage.Text = string.Empty;
            // Store TXN Cancellation request time
            Session["TXNCancellationRequestTime"] = DateTime.Now;
            // Generate new transaction ID for this request
            string transactionId = txnTransactionId.Text.Trim();
            string globalDeviceId = txnGlobalDeviceId.Text.Trim();
            string requestDatetime = txnRequestDatetime.Text.Trim();
            ...
            {
                lblTXNCancellationMessage.Text = "Please authorize first to obtain a private key.";
                lblTXNCancellationMessage.CssClass = "text-danger";
                return;
            }

            txnPrivateKey.Text = privateKey;

            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId) || string.IsNullOrEmpty(requestDatetime))
            {
                lblTXNCancellationMessage.Text = "Transaction ID, Global Device ID and Request DateTime are required.";
                lblTXNCancellationMessage.CssClass = "text-danger";
                return;
            }
            ...
            string postData = $"global_device_id={HttpUtility.UrlEncode(globalDeviceId)}&request_datetime={HttpUtility.UrlEncode(requestDatetime)}";

            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting TXN Cancellation for transaction: {transactionId}");

                TXNCancellationResponse txnResponse = PostTXNCancellation(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] PostTXNCancellation returned: {(txnResponse == null ? "NULL" : $"status={txnResponse.status}, message={txnResponse.message}")}");

                if (txnResponse == null)
                {
                    lblTXNCancellationMessage.Text = "Failed to get a response from the TXN cancellation endpoint.";
                    lblTXNCancellationMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblTXNCancellationMessage.Text = $"TXN cancellation failed: {txnResponse.message}";
                    lblTXNCancellationMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblTXNCancellationMessage.Text = "TXN cancellation accepted, but transaction status could not be retrieved.";
                    lblTXNCancellationMessage.CssClass = "text-warning";
                    return;
                }
                ...
                if (commandCancelled)
                {
                    ...
                    lblTXNCancellationMessage.Text = $"TXN cancellation failed. Command was cancelled by the meter (indv_status: 2, status_level: 4). Status: {stageText}.";
                    lblTXNCancellationMessage.CssClass = "text-danger";
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableTXNCancellationBtn", "disableTXNCancellationButton();", true);
                }
                else
                {
                    string stageText = AppCommon.GetStageDescription(maxStatusLevel);
                    lblTXNCancellationMessage.Text = $"TXN cancellation succeeded. Transaction status: {statusResponse.status}. Latest status level: {maxStatusLevel} ({stageText}).";
                    lblTXNCancellationMessage.CssClass = "text-success";
            ...
            {
                lblTXNCancellationMessage.Text = "HTTP Error: " + ex.Message;
                lblTXNCancellationMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblTXNCancellationMessage.Text = "Error: " + ex.Message;
                lblTXNCancellationMessage.CssClass = "text-danger";
            }
        }
        ...
        private TrackerPageContext BuildTrackerContext()
        {
            return new TrackerPageContext
            {
                ...
                GetGlobalDeviceId = () => txnGlobalDeviceId.Text.Trim(),
        ...
        private TXNCancellationResponse PostTXNCancellation(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<TXNCancellationResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"TXN cancellation request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class TXNCancellationResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public string message { get; set; }
...
protected void timerTables_Tick(object sender, EventArgs e)
        ...
        protected void SaveRemarks_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    SaveTestResult(tableName, "Fail", remarks, txnTransactionId.Text.Trim(), txnGlobalDeviceId.Text.Trim());
        ...
        {
            SaveTestResult(tableName, action, reason ?? string.Empty, txnTransactionId.Text.Trim(), txnGlobalDeviceId.Text.Trim());
        }
