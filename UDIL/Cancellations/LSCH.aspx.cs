    ...
    public partial class LSCH : UDIL.AuthenticatedPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    lsPrivateKey.Text = SessionManager.PrivateKey;
                }

                // Populate GlobalDeviceId and MSN from session if available
                if (SessionManager.HasGlobalDeviceId)
                {
                    lsGlobalDeviceId.Text = SessionManager.GlobalDeviceId;
                }

                // Generate unique transaction ID
                lsTransactionId.Text = AppCommon.GenerateTransactionId();
            }
        }



        protected void btnLSCHCancellation_Click(object sender, EventArgs e)
        {
            lblLSCHCancellationMessage.Text = string.Empty;
            // Store LSCH Cancellation request time
            Session["LSCHCancellationRequestTime"] = DateTime.Now;
            // Generate new transaction ID for this request
            string transactionId = lsTransactionId.Text.Trim();
            string globalDeviceId = lsGlobalDeviceId.Text.Trim();
            ...
            {
                lblLSCHCancellationMessage.Text = "Please authorize first to obtain a private key.";
                lblLSCHCancellationMessage.CssClass = "text-danger";
                return;
            }

            lsPrivateKey.Text = privateKey;

            if (string.IsNullOrEmpty(transactionId) || string.IsNullOrEmpty(globalDeviceId))
            {
                lblLSCHCancellationMessage.Text = "Transaction ID and Global Device ID are required.";
                lblLSCHCancellationMessage.CssClass = "text-danger";
                return;
            }
            ...
            string postData = $"global_device_id={HttpUtility.UrlEncode(globalDeviceId)}&type=lsch";

            try
            {
                System.Diagnostics.Debug.WriteLine($"[DEBUG] Starting LSCH Cancellation for transaction: {transactionId}");

                LSCHCancellationResponse lschResponse = PostLSCHCancellation(transactionId, privateKey, postData);

                System.Diagnostics.Debug.WriteLine($"[DEBUG] PostLSCHCancellation returned: {(lschResponse == null ? "NULL" : $"status={lschResponse.status}, message={lschResponse.message}")}");

                if (lschResponse == null)
                {
                    lblLSCHCancellationMessage.Text = "Failed to get a response from the LSCH cancellation endpoint.";
                    lblLSCHCancellationMessage.CssClass = "text-danger";
                    return;
                }

                if (lschResponse.status != "1")
                {
                    lblLSCHCancellationMessage.Text = $"LSCH cancellation failed: {lschResponse.message}";
                    lblLSCHCancellationMessage.CssClass = "text-danger";
                    return;
                }
                ...
                {
                    lblLSCHCancellationMessage.Text = "LSCH cancellation accepted, but transaction status could not be retrieved.";
                    lblLSCHCancellationMessage.CssClass = "text-warning";
                    return;
                }
                ...
                if (commandCancelled)
                {
                    ...
                    lblLSCHCancellationMessage.Text = $"LSCH cancellation failed. Command was cancelled by the meter (indv_status: 2, status_level: 4). Status: {stageText}.";
                    lblLSCHCancellationMessage.CssClass = "text-danger";
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "disableLSCHCancellationBtn", "disableLSCHCancellationButton();", true);
                }
                else
                {
                    string stageText = AppCommon.GetStageDescription(maxStatusLevel);
                    lblLSCHCancellationMessage.Text = $"LSCH cancellation succeeded. Transaction status: {statusResponse.status}. Latest status level: {maxStatusLevel} ({stageText}).";
                    lblLSCHCancellationMessage.CssClass = "text-success";
            ...
            {
                lblLSCHCancellationMessage.Text = "HTTP Error: " + ex.Message;
                lblLSCHCancellationMessage.CssClass = "text-danger";
            }
            catch (Exception ex)
            {
                lblLSCHCancellationMessage.Text = "Error: " + ex.Message;
                lblLSCHCancellationMessage.CssClass = "text-danger";
            }
        }
        ...
        private TrackerPageContext BuildTrackerContext()
        {
            return new TrackerPageContext
            {
                ...
                GetGlobalDeviceId = () => lsGlobalDeviceId.Text.Trim(),
        ...
        private LSCHCancellationResponse PostLSCHCancellation(string transactionId, string privateKey, string postData)
        {
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    return serializer.Deserialize<LSCHCancellationResponse>(responseContent);
                }
            }
            catch (WebException ex)
            {
                string body = AppCommon.ReadWebExceptionResponse(ex);
                throw new WebException($"LSCH cancellation request failed: {ex.Message}. Response body: {body}", ex);
            }
        }
        ...
        public class LSCHCancellationResponse
        {
            public string status { get; set; }
            public string transactionid { get; set; }
            public string message { get; set; }
...
protected void timerTables_Tick(object sender, EventArgs e)
        ...
        protected void SaveRemarks_Command(object sender, CommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    SaveTestResult(tableName, "Fail", remarks, lsTransactionId.Text.Trim(), lsGlobalDeviceId.Text.Trim());
        ...
        {
            SaveTestResult(tableName, action, reason ?? string.Empty, lsTransactionId.Text.Trim(), lsGlobalDeviceId.Text.Trim());
        }
